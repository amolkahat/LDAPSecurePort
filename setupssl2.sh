...
# adding port if not present
echo "Adding secure port"
semanage port -a -t ldap_port_t -p tcp $ldapsport
...
ldapmodify -x -h localhost -p $ldapport -D "cn=directory manager" -w $lpass <<EOF
